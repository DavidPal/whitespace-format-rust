...
pub enum OutputNewLineMarkerMode {
    #[default]
    #[clap(
        help = "Use new line marker that is the most common in each individual file. If no new line marker is present in the file, Linux '\\n' is used."
    )]
    Auto,
...
pub struct CommandLineArguments {
    #[arg(
        ...
        help = "Do not format files. Only report which files would be formatted. Exit code is zero if input is formatted correctly. Exit code is non-zero if formatting is required."
    )]
    pub check_only: bool,
    ...
    #[arg(long, default_value_t = String::from(UNMATCHABLE_REGEX),
    help = "Regular expression that specifies which files to exclude. The regular expression is evaluated on the path of each file. The default value is a regular expression that does not match anything.",
    long_help = "Regular expression that specifies which files to exclude. The regular expression is evaluated on the path of each file. The default value is a regular expression that does not match anything.\n\nExample #1: --exclude=\"(.jpeg|.png)$\" excludes files with '.jpeg' or '.png' extension.\nExample #2: --exclude=\".git/\" excludes all files in the '.git/' directory.")]
    pub exclude: String,

    #[arg(long, value_enum, default_value_t = OutputNewLineMarkerMode::Auto, help = "New line marker to use.")]
    ...
    #[arg(
        long,
        default_value_t = false,
        help = "Remove a new line marker from the end of the file if it is present. This option automatically implies the --remove_trailing_empty_lines option."
    )]
    ...
    #[arg(long, value_enum, default_value_t = TrivialFileReplacementMode::Ignore, help = "Replace files consisting of whitespace only.")]
    pub normalize_whitespace_only_files: TrivialFileReplacementMode,

    #[arg(long, value_enum, default_value_t = NonStandardWhitespaceReplacementMode::Ignore, help = "Replace or remove non-standard whitespace characters '\\v' and '\\f' in each file.")]
    pub normalize_non_standard_whitespace: NonStandardWhitespaceReplacementMode,

    #[arg(long, default_value_t = -1, help = "Replace tabs with spaces. The parameter specifies the number of spaces to use. If the parameter is negative, tabs are not replaced.")]
    pub replace_tabs_with_spaces: isize,

    /// List of files and/or directories to process.
